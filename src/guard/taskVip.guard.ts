...
import { AccessTokenConfig, TaskCommonNumber, TaskVipNumber } from 'src/config/util.config';
    ...
    async canActivate(context: ExecutionContext) {
        try {
            ...
            const { isVip } = await this.userRepository
                ...
                .getOne();
            if (!isVip) {
                //判断当前用户的每月发送定时任务文件超过3000
                return this.hasUserSentOverVipNumberFiles(sub, TaskVipNumber)
            }
            return this.hasUserSentOverVipNumberFiles(sub, TaskCommonNumber)
